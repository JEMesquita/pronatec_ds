...
class JarvisApp:
    ...
    def executar_assistente(self):
        ...
        ##Código Rascunho
        def obter_dados_apresentacao(local="São Paulo"):
          hora_atual = datetime.datetime.now().hour
          data_atual = datetime.datetime.now().strftime("%d/%m/%Y")
          hora_atual_formatada = datetime.datetime.now().strftime("%H:%M")
          temperatura = obter_temperatura(local)  # Assumindo que essa função já existe
          return hora_atual, data_atual, hora_atual_formatada, temperatura

        def construir_mensagem(saudacao, data_atual, hora_atual_formatada, temperatura, nome_usuario="Chefe"):
          return f"{saudacao} {nome_usuario}, hoje é {data_atual}, são {hora_atual_formatada}, {temperatura}. Me chamo Jarvis."


        def apresentacao(nome_usuario="Chefe", local="São Paulo"):
          animar_circulo()
          hora_atual, data_atual, hora_atual_formatada, temperatura = obter_dados_apresentacao(local)
          ...
          if 0 <= hora_atual < 12:
            ...
            saudacao = "Boa noite"
            mensagem = construir_mensagem(saudacao, data_atual, hora_atual_formatada, temperatura, nome_usuario)
            falar(mensagem)
                            ...
                            try:
                                ...
                                if "pesquisar" in comando:
                                    ...
                                    try:
                                        ...
                                        caminho_arquivo = (r"C:\Users\Joao.Mesquita\Área de Trabalho")
                                ...
                                falar("Perdão, não entendi o que o senhor disse.")
                    except sr.UnknownValueError:
                        pass
            return False

        def obter_temperatura(cidade):
            api_key = "d0283f5d7d16ff7b5f5ebd4e6d46be72"
        ...
        def obter_previsao(cidade):
            chave_api = "d0283f5d7d16ff7b5f5ebd4e6d46be72"
        ...
        def pesquisar_no_google(pesquisa):
            ...
            google_api_key = "AIzaSyBm1r_110h_B_sTkAoL8h0b_mEFIylixIg"
